...
const Game1 = props => {
    ...
    const updatePlayng = (id) => {
        axios.put("http://localhost:8000/api/player/game1/" + id, { "status": "Playing" })
            .then(res => console.log("resault", res))
            .catch(err => console.log("error", err))
    }
    const updateNotPlaying = (id) => {
        axios.put("http://localhost:8000/api/player/game1/" + id, { "status": "NotPlaying" })
            .then(res => console.log("resault", res))
            .catch(err => console.log("error", err))

    }
    const updateUndecided = (id) => {
        axios.put("http://localhost:8000/api/player/game1/" + id, { "status": "Undecided" })
            .then(res => console.log("resault", res))
            .catch(err => console.log("error", err))
